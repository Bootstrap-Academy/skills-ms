...
async def preserved(mocker, source='source'):
...
def authority(right, subject='fresh'):
    original={key:value for key,value in right.items() if key not in ('current_subject','generation')}
    return {'id':str(uuid4()),'source':'skills','successor':subject,'original_contract':right['id'],
            'original_scope':original,'claimant_authorization':{'source_subject':right['source_user_id']},
            'purpose':'existing_course_continuation','new_purchase':False}


async def test__same_right_delivery_is_usable_and_replay_does_not_resurrect(mocker):
    right=await preserved(mocker);grant=authority(right)
    ...
    premium=mocker.patch('api.endpoints.course.has_premium',AsyncMock(side_effect=AssertionError('No new Premium needed')))
    async with db_context():
        await course_routes.has_course_access.dependency(course=COURSES['course'],user=User(id='fresh',email_verified=True,admin=False))
    ...
    successor=authority(right,'next-fresh');response.return_value=successor
    async with db_context():
        resumed=await retained_rights.deliver('source',successor['id'])
        assert resumed['state']=='granted'
    async with db_context():
        await course_routes.has_course_access.dependency(course=COURSES['course'],user=User(id='next-fresh',email_verified=True,admin=False))
        assert await db.all(filter_by(models.CoursePurchase,user_id='next-fresh'))==[]
        assert not await db.exists(filter_by(models.CourseAccess,user_id='fresh'))


async def test__changed_or_unavailable_admission_keeps_original_entitlement(mocker):
    right=await preserved(mocker);grant=authority(right)
    response=mocker.patch('api.services.retained_rights.successor_authority',AsyncMock(side_effect=[grant,None]))
    with pytest.raises(HTTPException) as denied:
        async with db_context():await retained_rights.deliver('source',grant['id'])
    ...
    response.side_effect=None;response.return_value=grant
    with pytest.raises(HTTPException):
        async with db_context():await retained_rights.deliver('foreign',grant['id'])
    async with db_context():
        assert not await db.exists(filter_by(models.CourseAccess,user_id='fresh'))
        assert (await retained_rights.list_rights('source'))[0]['original']==right['original']


async def test__rolled_back_delivery_and_missing_course_do_not_satisfy_right(mocker):
    right=await preserved(mocker);grant=authority(right)
