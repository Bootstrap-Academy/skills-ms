...
async def preserve_before_erasure(user_id: str) -> None:
    ...
    access = {
        ...
        for row in await db.all(
            filter_by(CourseAccess, user_id=user_id).with_for_update().execution_options(populate_existing=True)
        )
    }
    started = {
        row.course_id
        for row in await db.all(
            filter_by(LastWatch, user_id=user_id).with_for_update().execution_options(populate_existing=True)
        )
    }
    continued = await db.all(
        filter_by(RetainedCourseRight, current_subject=user_id)
        .with_for_update()
    ...
    for grant in await db.all(
        filter_by(CourseRightGrant, subject=user_id)
        .where(CourseRightGrant.state == "granted")
        .with_for_update()
        .execution_options(populate_existing=True)
    ):
    ...
    for course_id in sorted((access | started) - continued_courses):
        ...
        purchases = await db.all(
            filter_by(CoursePurchase, user_id=user_id, course_id=course_id)
            .with_for_update()
            .execution_options(populate_existing=True)
        )
        await db.add(
            RetainedCourseRight(
                ...
                original={
                    ...
                    "scope": (
                        "Existing course admission at erasure; "
                        "no original payment, performance, duration or new terms inferred"
                    ),
...
def original(right: RetainedCourseRight) -> dict[str, Any]:
    return {
        "id": right.id,
        "source_user_id": right.source_user_id,
...
async def list_rights(user_id: str) -> list[dict[str, Any]]:
    return [
        original(right) | {"current_subject": right.current_subject, "generation": right.generation}
        for right in await db.all(
            filter_by(RetainedCourseRight, source_user_id=user_id).order_by(RetainedCourseRight.id)
        )
    ]
    ...
    try:
        async with InternalService.SHOP.client as client:
            ...
            response = await client.post(
                "/claims/course_successor_authority", json={"grant_id": grant_id, "source_subject": source_user_id}
            )
        if response.status_code != 200:
            raise HTTPException(503, "Current course continuation admission unavailable")
        value: dict[str, Any] | None = response.json()
        if value is None:
            return None
        if (
            value.get("id") != grant_id
            or value.get("source") != "skills"
            or value.get("purpose") != "existing_course_continuation"
...
async def deliver(source_user_id: str, grant_id: str) -> dict[str, Any]:
    ...
    if prior is not None:
        ...
        prior = await db.first(
            filter_by(CourseRightGrant, id=grant_id).with_for_update().execution_options(populate_existing=True)
        )
        if prior is None:
    ...
    right = await db.first(
        filter_by(RetainedCourseRight, id=authority["original_contract"], source_user_id=source_user_id)
        .with_for_update()
        .execution_options(populate_existing=True)
    )
    ...
    if (
        ...
        or not source_guard.deleted
        or original(right) != authority["original_scope"]
    ):
